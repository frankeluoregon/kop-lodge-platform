...
function svgOpen(vbW, vbH, title) {
  ...
  <text x="${vbW/2}" y="50" class="subtitle">Knights of Pythias Lodge &#x2014; 1 px = 1 in</text>
`;
}
...
// ============================================================
// 1. DINING HALL (Dining + Kitchen)
// Dining: 558 x 401
// Kitchen L-shape: upper wall x=558 y=0..274, step y=272 x=558..685
//   lower right wall x=683 y=272..401, bottom y=401 x=0..685
// EXIT opening at top-right of Kitchen
// ============================================================
(function() {
  const rW = 685, rH = 401;
  ...
  // Walls - Dining outer
  svg += `  <!-- Dining walls -->\n`;
  svg += `  <line x1="${ox}" y1="${oy}" x2="${ox + 558*s}" y2="${oy}" class="wall"/>\n`; // top
  svg += `  <line x1="${ox}" y1="${oy}" x2="${ox}" y2="${oy + 401*s}" class="wall"/>\n`; // left
  svg += `  <line x1="${ox}" y1="${oy + 401*s}" x2="${ox + 685*s}" y2="${oy + 401*s}" class="wall"/>\n`; // bottom full

  // Dining/Kitchen divider wall (with door gap near bottom)
  // Wall y=0 to y=230 (continuous), gap y=230 to y=272 (door), then meets step wall
  svg += `  <line x1="${ox + 558*s}" y1="${oy}" x2="${ox + 558*s}" y2="${oy + 230*s}" class="wall"/>\n`;
  // Door arc: hinge at y=230 on x=558, swings right into Kitchen
  svg += doorArc(ox + 558*s, oy + 230*s, 36*s, 'right-down') + '\n';

  // Kitchen step wall y=272
  svg += `  <!-- Kitchen walls -->\n`;
  svg += `  <line x1="${ox + 558*s}" y1="${oy + 272*s}" x2="${ox + 685*s}" y2="${oy + 272*s}" class="wall"/>\n`;
  // Kitchen right wall lower
  svg += `  <line x1="${ox + 683*s}" y1="${oy + 272*s}" x2="${ox + 683*s}" y2="${oy + 401*s}" class="wall"/>\n`;

  // EXIT at top - short wall stubs forming doorway at top of Kitchen
  // EXIT opens upward from around x=558..600
  const exitX1 = ox + 563*s, exitX2 = ox + 603*s;
  svg += `  <line x1="${exitX1}" y1="${oy}" x2="${exitX1}" y2="${oy - 15*s}" class="wall-thin"/>\n`;
  svg += `  <line x1="${exitX2}" y1="${oy}" x2="${exitX2}" y2="${oy - 15*s}" class="wall-thin"/>\n`;
  // Door arc swinging outward (upward)
  svg += doorArc(exitX1, oy, 36*s, 'right-up') + '\n';

  // Room labels
  svg += `  <text x="${ox + 279*s}" y="${oy + 195*s}" class="room-label">Dining</text>\n`;
  svg += `  <text x="${ox + 620*s}" y="${oy + 180*s}" class="room-label" font-size="18">Kitchen</text>\n`;

  // Dimensions
  dimReset();
  dimH(ox, ox + 558*s, oy, 558, true);
  dimH(ox + 558*s, ox + 685*s, oy + 272*s, 127, true);
  dimV(ox, oy, oy + 401*s, 401, true);
  dimV(ox + 683*s, oy + 272*s, oy + 401*s, 129, false);
  dimH(ox, ox + 685*s, oy + 401*s, 685, false);
...
(function() {
  ...
  // Two inswinging doors on bottom wall, evenly spaced across Grand Hall (404")
  const door1X = ox + (404 / 3) * s;
  const door2X = ox + (404 * 2 / 3) * s;
  const bottomY = oy + 486 * s;
  svg += doorArc(door1X, bottomY, 36*s, 'right-up') + '\n';
  svg += doorArc(door2X, bottomY, 36*s, 'right-up') + '\n';
